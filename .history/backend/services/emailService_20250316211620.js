...
class EmailService {
  constructor() {
    this.transporter = nodemailer.createTransport({
      ...
      pool: true, // Kích hoạt pooling để tăng tốc độ
      maxConnections: 5, // Số kết nối tối đa trong pool
      maxMessages: 100, // Số lượng message tối đa mỗi kết nối
      rateDelta: 1000, // Thời gian giữa các lần gửi
      rateLimit: 5, // Số lượng emails một lúc
      auth: {
  ...
  async sendVerificationOTP(user, otp) {
    // Kiểm tra xem user.email có tồn tại không
    if (!user || !user.email) {
      console.error('Lỗi: Email người nhận không hợp lệ', user);
      throw new Error('Email người nhận không hợp lệ');
    }
    
    console.log(`Chuẩn bị gửi OTP ${otp} đến ${user.email}`);
    
    const mailOptions = {
      from: `"MangaCorner" <${process.env.EMAIL_USER}>`,
      to: user.email, // Đảm bảo email người nhận hợp lệ
    ...
    try {
      console.log('Thông tin gửi email:', {
