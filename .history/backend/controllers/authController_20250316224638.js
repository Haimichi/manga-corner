...
const authController = {
  ...
  login: catchAsync(async (req, res, next) => {
    try {
      console.log('Đang xử lý đăng nhập cho:', req.body);
  ...
  register: catchAsync(async (req, res, next) => {
    try {
      ...
      if (!email || !password || !username) {
        return res.status(400).json({
          status: 'error',
          message: 'Vui lòng cung cấp email, mật khẩu và tên người dùng'
        });
      }
      ...
      console.log('Đang lưu người dùng mới vào MongoDB:', newUser);
      
      // Lưu người dùng vào database
      await newUser.save();
      console.log('Đã lưu người dùng thành công');
      
      // Gửi email xác thực
      try {
        await emailService.sendVerificationOTP(newUser, otp);
        console.log('Đã gửi OTP thành công đến email:', email);
      } catch (emailError) {
        console.error('Lỗi khi gửi email OTP:', emailError);
        // Vẫn tiếp tục xử lý mặc dù gửi email thất bại
      }
