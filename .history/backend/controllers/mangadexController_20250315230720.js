...
exports.searchManga = async (req, res) => {
  ...
  try {
    ...
    // Xây dựng URL với tham số query thủ công
    let apiUrl = `${config.MANGADEX_API}/manga?limit=${limit}&offset=${offset}`;
    apiUrl += `&order[${sort}]=${order}`;
    apiUrl += `&translatedLanguage[]=${language}`;
    apiUrl += `&contentRating[]=safe&contentRating[]=suggestive&contentRating[]=erotica`;
    apiUrl += `&includes[]=cover_art&includes[]=author&includes[]=artist`;
...
exports.getMangaChapters = async (req, res) => {
  try {
    ...
    // Tạo đối tượng order phù hợp
    const orderParams = { chapter: 'desc' };

    const url = `${config.MANGADEX_API}/manga/${mangaId}/feed`;
    const response = await axios.get(url, {
      params: {
        limit: parseInt(limit),
        offset: parseInt(offset),
        translatedLanguage: [language],
        // Chuyển order thành chuỗi JSON
        order: JSON.stringify(orderParams),
