...
exports.getLatestManga = async (req, res) => {
  try {
    ...
    // Xử lý tham số đúng cách, hỗ trợ cả hai định dạng
    let page = parseInt(req.query.page) || parseInt(req.query['page[page]']) || 1;
    let limit = parseInt(req.query.limit) || parseInt(req.query['page[limit]']) || 20;
    const offset = (page - 1) * limit;
    
    console.log(`Tham số đã xử lý: page=${page}, limit=${limit}, offset=${offset}`);
    ...
    console.log(`Gọi MangaDex API với params:`, JSON.stringify(params));
    
    // Gọi API MangaDex
    const response = await axios.get(`${MANGADEX_API}/manga`, { params });
    
    // Log thêm thông tin về dữ liệu nhận được
    console.log(`Nhận được ${response.data.data?.length || 0} manga từ API`);
...
exports.getPopularManga = async (req, res) => {
  try {
    ...
    let page = parseInt(req.query.page) || parseInt(req.query['page[page]']) || 1;
    let limit = parseInt(req.query.limit) || parseInt(req.query['page[limit]']) || 20;
    let language = req.query.language || req.query['page[language]'] || 'vi';
    const offset = (page - 1) * limit;
    
    console.log(`Tham số đã xử lý: page=${page}, limit=${limit}, offset=${offset}, language=${language}`);
