...
exports.getLatestManga = async (req, res) => {
  try {
    ...
    // Lấy và đảm bảo tham số là số nguyên hợp lệ
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 20;
    const offset = (page - 1) * limit;
    
    // Kiểm tra tham số và log cho debug
...
exports.getPopularManga = async (req, res) => {
  try {
    ...
    // Gọi API MangaDex
    const response = await axios.get(`${process.env.MANGADEX_API_URL}/manga`, { params });
