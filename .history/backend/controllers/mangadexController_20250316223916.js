...
exports.getMangaDetails = async (req, res) => {
  try {
    const { id } = req.params;
    console.log(`Đang lấy thông tin manga ID: ${id}`);

    // Gọi API MangaDex để lấy thông tin chi tiết manga
    const response = await axios.get(`${MANGADEX_API}/manga/${id}?includes[]=cover_art&includes[]=author&includes[]=artist`);
    
    // Trả về dữ liệu từ MangaDex
    res.json(response.data);
  } catch (error) {
    console.error(`Lỗi khi lấy thông tin manga ${req.params.id}:`, error.response?.data || error.message);
    
    // Trả về lỗi cho client
    res.status(error.response?.status || 500).json({
      status: 'error',
      message: `Không thể lấy thông tin manga: ${error.message}`,
...
exports.getMangaChapters = async (req, res) => {
  try {
    const { id } = req.params;
    const { limit = 30, offset = 0, translatedLanguage = 'en' } = req.query;
    
    // Gọi API MangaDex để lấy danh sách chapter
    ...
    console.error(`Lỗi khi lấy chapters của manga ${req.params.id}:`, error.response?.data || error.message);
    
    // Trả về lỗi cho client
    res.status(error.response?.status || 500).json({
      status: 'error',
      message: `Không thể lấy danh sách chapter: ${error.message}`,
      details: error.response?.data
    });
  }
};
