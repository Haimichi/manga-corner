...
const MangaDetail = () => {
  ...
  useEffect(() => {
    console.log('mangaApi methods available:', Object.keys(mangaApi));
  }, []);
  ...
  const fetchMangaDetails = async () => {
    try {
      ...
      // Thử gọi từ API service của bạn
      let data;
      try {
        // Kiểm tra xem phương thức nào tồn tại
        if (typeof mangaApi.getMangaDetails === 'function') {
          data = await mangaApi.getMangaDetails(id);
        } else if (typeof mangaApi.getMangaDetail === 'function') {
          data = await mangaApi.getMangaDetail(id);
        } else {
          throw new Error('Không tìm thấy phương thức API phù hợp');
        }
      } catch (apiError) {
        console.error('Lỗi khi gọi API từ service:', apiError);
        
        // Thử gọi trực tiếp từ MangaDex API
      ...
      console.log('MangaDetail - Dữ liệu manga:', data);
      
      setManga(data.manga);
      setChapters(data.chapters || []);
      setSelectedLanguage(data.language || 'vi');
