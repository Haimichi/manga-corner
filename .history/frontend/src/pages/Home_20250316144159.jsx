...
const Home = () => {
  ...
  const fetchLatestManga = useCallback(async () => {
    try {
      ...
      const response = await mangaApi.getLatest(1, 18);
      dispatch(fetchLatestMangaAction({ 
        limit: 18, 
        offset: 0,
        sort: 'updatedAt',
        order: 'desc'
  ...
  const fetchPopularManga = useCallback(async () => {
    try {
      ...
      const response = await mangaApi.getPopular(1, 18, 'vi');
      dispatch(fetchPopularMangaAction({ 
        limit: 18, 
        offset: 0 
      }));
    } catch (error) {
