...
import { getMangaDetailAsync, fetchMangaChapters, resetChapters, fetchMangaDetails } from '../features/mangadex/mangadexSlice';
...
const RecommendedManga = ({ relatedManga, loading }) => {
  return (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {relatedManga && relatedManga.length > 0 ? (
            ...
            <div className="col-span-6 text-center py-8 text-gray-400">
              <FaRandom className="mx-auto text-5xl mb-3 opacity-30" />
              <p>Không tìm thấy truyện tương tự. Hãy thử lại sau.</p>
            </div>
          )}
        </div>
      )}
...
const ChapterList = ({ mangaId, chapters, loading }) => {
  ...
  const [chaptersPerPage, setChaptersPerPage] = useState(10); // Mặc định hiển thị 10 chapter
...
const MangaDetail = () => {
  ...
  useEffect(() => {
    if (id) {
      ...
      dispatch(getMangaDetailAsync(id));
  ...
  useEffect(() => {
    if (manga && manga.attributes?.tags && manga.attributes.tags.length > 0) {
      ...
      // Lấy tag ids từ manga hiện tại
      const tagIds = manga.attributes.tags.map(tag => tag.id).slice(0, 3);
      
      // Gọi API từ MangaDex
      axios.get(`${API_BASE_URL}/manga`, {
        params: {
          limit: 6,
          includedTags: tagIds,
      ...
      .then(response => {
        ...
        if (response.data && response.data.data) {
          const relatedMangaList = response.data.data.map(manga => {
            const cover = manga.relationships?.find(rel => rel.type === 'cover_art');
            const coverFileName = cover?.attributes?.fileName;
            
            return {
              id: manga.id,
              title: getLocalizedValue(manga.attributes.title),
              coverImage: coverFileName 
                ? `https://uploads.mangadex.org/covers/${manga.id}/${coverFileName}`
                : 'https://api.dicebear.com/7.x/shapes/svg?seed=' + manga.id,
              rating: (Math.random() * (5 - 4) + 4).toFixed(1),
              genres: manga.attributes.tags
                .slice(0, 3)
                .map(tag => getLocalizedValue(tag.attributes.name))
            };
          });
          
          setRelatedManga(relatedMangaList);
        } else {
          setRelatedManga([]);
        }
      ...
      .catch(error => {
        console.error('Lỗi khi lấy truyện tương tự:', error);
        setRelatedManga([]);
        setLoadingRelated(false);
      });
    }
    ...
    return (
      <div className="container mx-auto px-4 py-10">
        <div className="max-w-6xl mx-auto bg-red-900 border border-red-800 rounded-lg p-6 text-white">
          ...
          <button
            onClick={() => dispatch(getMangaDetailAsync(id))}
    ...
    <div className="relative bg-gray-900 min-h-screen pb-12">
      ...
      <div className="container mx-auto px-4 relative z-10 pt-10">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row gap-6">
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-3 mb-8">
                <Link 
                  to={`/manga/${id}/chapter/${manga.firstChapterId || ''}`}
          ...
          {/* Danh sách chapter */}
          <div className="mt-10">
            <ChapterList mangaId={id} chapters={chapters} loading={chaptersLoading} />
          </div>

          {/* Phần bình luận */}
          <div className="mt-10">
            <CommentsSection mangaId={id} />
          </div>

          {/* Phần truyện đề xuất */}
          <div className="mt-10">
            <RecommendedManga relatedManga={relatedManga} loading={loadingRelated} />
          </div>
