...
const MangaDetail = () => {
  ...
  useEffect(() => {
    if (manga && manga.attributes?.tags && manga.attributes.tags.length > 0) {
      ...
      // Lấy tag ids từ manga hiện tại để tìm truyện tương tự
      const tagIds = manga.attributes.tags.map(tag => tag.id).slice(0, 3); // Lấy tối đa 3 tags phổ biến nhất
      
      // Gọi API để lấy truyện tương tự
      axios.get(`${API_BASE_URL}/manga`, {
        params: {
          ...
          includes: ['cover_art'],
          excludedIds: [mangaId], // Loại trừ manga hiện tại
        ...
        if (response.data && response.data.data) {
          const relatedMangaList = response.data.data.map(manga => {
            ...
            return {
              ...
              rating: (Math.random() * (5 - 4) + 4).toFixed(1), // Hiện tại MangaDex không có rating, nên tạm thời dùng số ngẫu nhiên
