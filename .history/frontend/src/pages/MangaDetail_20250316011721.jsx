...
const MangaDetail = () => {
  ...
  useEffect(() => {
    if (mangaId) {
      console.log('Đang lấy thông tin manga với ID:', mangaId);
  ...
  const groupChaptersByNumber = (chaptersArray) => {
    ...
    if (!chaptersArray || !Array.isArray(chaptersArray)) {
      console.log('Chapters không phải mảng:', chaptersArray);
      return grouped;
    }
    ...
    chaptersArray.forEach(chapter => {
      try {
        if (!chapter || !chapter.attributes) {
          console.warn('Chapter không hợp lệ:', chapter);
          return;
        }
  ...
  console.log("Chapters từ Redux:", chapters);
  const isChaptersArray = Array.isArray(chapters);
  console.log("Chapters có phải là mảng không:", isChaptersArray);
  
  // Chỉ gọi khi chapters là mảng
  ...
  const getScanlationGroupName = (chapter) => {
    try {
      if (!chapter || !chapter.relationships) return 'Unknown Group';
  ...
  console.log('Thông tin manga hiện tại:', manga);
  console.log('Danh sách chapters hiện tại:', chapters);
