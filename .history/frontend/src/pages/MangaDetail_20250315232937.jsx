...
import { getMangaDetailAsync, getMangaChaptersAsync } from '../features/mangadex/mangadexSlice';
...
const MangaDetail = () => {
  ...
  useEffect(() => {
    if (mangaId) {
      ...
      dispatch(getMangaDetailAsync(mangaId))
        .then(() => {
          dispatch(getMangaChaptersAsync(mangaId));
        })
        .finally(() => {
  ...
  const handleLoadMoreChapters = () => {
    if (chapterPagination.hasMore && !loading) {
      dispatch(getMangaChaptersAsync({ 
        mangaId, 
