...
const MangaDetail = () => {
  ...
  useEffect(() => {
    if (mangaId) {
      // Force mới để tránh vấn đề cache
      dispatch(getMangaDetailAsync(mangaId));
      dispatch(fetchMangaChapters(mangaId));
      
      // Thêm timestamp để buộc browser không sử dụng cache
      const timestamp = new Date().getTime();
      console.log(`Force refresh chapters at ${timestamp}`);
      
      // Sau 1 giây, thử lại nếu không có dữ liệu
      const timer = setTimeout(() => {
        if (!chapters || !chapters.data || chapters.data.length === 0) {
          console.log('Thử lại lấy chapter...');
          dispatch(fetchMangaChapters(mangaId));
        }
      }, 1000);
