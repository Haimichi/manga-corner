...
const Home = () => {
  ...
  const fetchLatestMangaData = useCallback(async () => {
    try {
      ...
      console.log('Home - Đang tải manga mới nhất...');
      
      const response = await mangaApi.getLatest({ page: 1, limit: 18 });
      console.log('Home - Response từ API:', response);
      
      // Kiểm tra cấu trúc response
      if (response && response.data) {
        // Log chi tiết cấu trúc dữ liệu
        console.log('Home - Cấu trúc response.data:', typeof response.data, Array.isArray(response.data.data));
        
        // Kiểm tra nhiều trường hợp cấu trúc dữ liệu
        if (Array.isArray(response.data.data)) {
          console.log('Home - Cấu trúc 1: response.data.data là mảng');
          ...
          console.log('Home - Cấu trúc 2: response.data là mảng');
          dispatch(fetchLatestManga({ 
            limit: 18, 
            offset: 0,
            sort: 'updatedAt',
          ...
          console.log('Home - Cấu trúc 3: response.data là object');
          dispatch(fetchLatestManga({ 
            limit: 18, 
            offset: 0,
            sort: 'updatedAt',
        ...
        console.error('Home - Không có dữ liệu hợp lệ:', response);
  ...
  const fetchPopularMangaData = useCallback(async () => {
    try {
      ...
      const response = await mangaApi.getPopular({ page: 1, limit: 18, language: 'vi' });
      console.log('Popular manga response:', response);
      
      // Cập nhật state local trước
      if (response && response.data && response.data.data) {
        dispatch(fetchPopularManga({ limit: 18, offset: 0 }));
      } else {
        dispatch(fetchPopularManga({ limit: 18, offset: 0 }));
      }
