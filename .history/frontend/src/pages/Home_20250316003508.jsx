...
const Home = () => {
  ...
  // Thêm useEffect để trích xuất màu từ hình ảnh
  useEffect(() => {
    if (hasPopularManga) {
      const featuredManga = popularManga.slice(0, 6);
      
      const extractColors = async () => {
        const colorPromises = featuredManga.map(async (manga) => {
          const coverUrl = getHighResolutionCover(manga);
          try {
            const palette = await Vibrant.from(coverUrl).getPalette();
            // Lấy màu chủ đạo và màu phụ từ palette
            return {
              primary: palette.Vibrant ? palette.Vibrant.hex : '#1a202c',
              secondary: palette.Muted ? palette.Muted.hex : '#4a5568',
              dark: palette.DarkVibrant ? palette.DarkVibrant.hex : '#2d3748'
            };
          } catch (error) {
            console.error("Không thể trích xuất màu từ ảnh:", coverUrl);
            return {
              primary: '#1a202c',
              secondary: '#4a5568',
              dark: '#2d3748'
    ...
    return (
      <div className="relative h-[500px] md:h-[550px] lg:h-[600px]">
        ...
        {featuredManga.map((manga, index) => {
          // Lấy màu cho slide hiện tại hoặc sử dụng màu mặc định
        ...
        <div className="absolute bottom-8 left-0 right-0 flex justify-center space-x-3 z-20">
          {featuredManga.map((_, index) => {
            // Sử dụng màu từ ảnh cho các chỉ báo slide
            const dotColor = slideColors[index]?.primary || '#ffffff';
