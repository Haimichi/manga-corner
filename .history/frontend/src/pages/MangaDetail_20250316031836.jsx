...
const MangaDetail = () => {
  ...
  useEffect(() => {
    if (mangaId) {
      ...
      // Thêm dòng này để lấy danh sách chapter
    ...
    const fetchRelatedManga = async () => {
      if (manga && manga.attributes?.tags && manga.attributes.tags.length > 0) {
        try {
          ...
          // Lấy các tag IDs từ manga hiện tại
          const tagIds = manga.attributes.tags.map(tag => tag.id).slice(0, 3);
          
          // Logic mô phỏng: Khi tích hợp thực tế, thay thế phần này bằng gọi API
          // Giả lập dữ liệu tương tự với tag giống với manga hiện tại
          setTimeout(() => {
