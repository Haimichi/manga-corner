...
const Home = () => {
  ...
  const fetchLatestMangaData = useCallback(async () => {
    try {
      ...
      // Gọi API với params đúng định dạng
      const response = await mangaApi.getLatest({ 
        page: 1, 
        limit: 18 
      });
      
      console.log('Home - Latest manga response:', response);
      
      // Kiểm tra dữ liệu
      if (response && response.data && Array.isArray(response.data.data)) {
        console.log('Home - Có dữ liệu manga, số lượng:', response.data.data.length);
        ...
        console.error('Home - Cấu trúc dữ liệu không hợp lệ:', response);
