          ...
          <div className="flex flex-col lg:flex-row gap-8">
                                    ...
                                    <div className="mt-4 space-y-4 pl-6 border-l-2 border-gray-200">
                                      {comment.replies.map(reply => (
                                        <div key={reply.id} className="flex items-start space-x-3">
                                          ...
                                          <div className="flex-1">
                                            <div className="flex items-center">
                                              <h4 className="font-medium text-gray-900">{reply.user.username}</h4>
                                              <span className="ml-2 text-xs text-gray-500">{formatDate(reply.date)}</span>
                                            </div>
            ...
            <div className="w-full lg:w-1/3">
              ...
              <div className="bg-white rounded-lg shadow-md p-6 mb-8">
                <h3 className="text-lg font-bold text-gray-900 mb-4">Manga liên quan</h3>
                      ...
                      <div className="w-16 h-20 flex-shrink-0 overflow-hidden rounded">
                        <img 
                          src={`https://uploads.mangadex.org/covers/${manga.id}/${
                            manga.relationships?.find(rel => rel.type === 'cover_art')?.attributes?.fileName || 'cover.jpg'
                          }`}
              ...
              <div className="bg-white rounded-lg shadow-md p-6">
                <h3 className="text-lg font-bold text-gray-900 mb-4">Từ cùng tác giả</h3>
                <div className="space-y-4">
                  {relatedManga.slice(0, 2).map(manga => (
                    <Link 
                      key={manga.id}
                      to={`/manga/${manga.id}`}
                      className="flex items-center group"
                  ...
                  <button className="w-full text-primary-600 hover:text-primary-700 text-center text-sm font-medium mt-2">
                    Xem tất cả tác phẩm
                  </button>
                </div>
              </div>
