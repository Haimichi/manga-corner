...
const Home = () => {
  ...
  const fetchLatestMangaData = useCallback(async () => {
    try {
      ...
      // Thêm kiểm tra dữ liệu nhận được
      if (response && response.data) {
        // Gọi action với đầy đủ dữ liệu
        dispatch(fetchLatestManga({ 
          limit: 18, 
          offset: 0,
          sort: 'updatedAt',
          order: 'desc',
          // Truyền dữ liệu nhận được vào action để tránh gọi API thêm lần nữa
          responseData: response.data
        }));
      } else {
        // Vẫn dispatch action nhưng báo rõ là không có dữ liệu
        dispatch(fetchLatestManga({ 
          limit: 18, 
          offset: 0,
          sort: 'updatedAt',
      ...
      // Vẫn dispatch để cập nhật state với lỗi
      dispatch(fetchLatestManga({ 
        limit: 18, 
        offset: 0,
        sort: 'updatedAt',
