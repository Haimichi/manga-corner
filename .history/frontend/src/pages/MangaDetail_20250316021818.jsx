...
const MangaDetail = () => {
  ...
  useEffect(() => {
    if (mangaId) {
      // Reset state chapters
      dispatch(resetChapters());
      
      // Fetch data với timestamp để tránh cache
      dispatch(getMangaDetailAsync(mangaId));
      dispatch(fetchMangaChapters(mangaId));
      
      // Retry fetch nếu không nhận được data sau 2 giây
      const timer = setTimeout(() => {
        const chaptersData = chapters?.data;
        if (!chaptersData || chaptersData.length === 0) {
          console.log('Chapter không có dữ liệu, thử lại...');
          dispatch(fetchMangaChapters(mangaId));
        }
      }, 2000);
