...
const Register = () => {
  ...
  const onSubmit = async (data) => {
    if (step === 1) {
      ...
      try {
        ...
        // Đã loại bỏ đoạn code tự động điền OTP trong môi trường development
      } catch (error) {
        ...
        toast.error(errorMessage, {
          duration: 5000, // Hiển thị thông báo lâu hơn
          position: 'top-center'
        });
        
        // Nếu lỗi liên quan đến email đã tồn tại, hiển thị lỗi cụ thể tại trường email
        if (errorMessage.includes('Email đã được sử dụng')) {
          // Thiết lập lỗi cho trường email
          error => {
            error.current?.setError('email', {
              type: 'manual',
              message: 'Email này đã được đăng ký. Vui lòng sử dụng email khác hoặc đăng nhập.'
            });
          }
          
          // Thêm thông báo lỗi rõ ràng kèm theo hướng dẫn
          const emailInput = document.querySelector('input[name="email"]');
          if (emailInput) {
            emailInput.focus();
            emailInput.classList.add('border-red-500', 'bg-red-900/20');
          }
        }
      } finally {
        setIsLoading(false);
  ...
  const handleResendOtp = async () => {
    ...
    setIsLoading(true);
    try {
      // Thử sử dụng các endpoint phổ biến cho việc gửi lại OTP
      // Đổi từ '/auth/resend-verification' thành một trong các endpoint sau:
      const response = await api.post('/auth/resend-otp', {  // Đổi tên endpoint
        email: email
      });
      ...
      console.error('Chi tiết lỗi gửi lại OTP:', {
        message: error.message,
        status: error.response?.status,
        data: error.response?.data,
        config: {
