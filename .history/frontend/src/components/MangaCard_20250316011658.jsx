...
const MangaCard = ({ manga }) => {
  ...
  const getCoverImage = () => {
    try {
      if (!manga || !manga.id || !manga.relationships) return '/images/default-cover.jpg';
  ...
  // Kiểm tra có manga hợp lệ không - DI CHUYỂN phần này xuống dưới useEffect
  const isValidManga = manga && manga.id;
  ...
  const isUpdatedRecently = () => {
    try {
      if (!manga || !manga.attributes || !manga.attributes.updatedAt) return false;
  ...
  // Trả về early return sau khi đã định nghĩa tất cả hooks
  if (!isValidManga) {
    console.error("MangaCard nhận được manga không hợp lệ:", manga);
    return null;
  }
  
  // Sửa các đường dẫn placeholder image
  // Thay vì sử dụng via.placeholder.com (không hoạt động), hãy dùng một dịch vụ khác
  const defaultImage = "https://placehold.co/350x600/3B82F6/FFFFFF?text=Manga+Corner";
  ...
  return (
    ...
    >
      <div className="relative pb-[140%] overflow-hidden bg-gray-100 dark:bg-gray-700">
        ...
        <img 
          src={imageLoaded ? coverImage : defaultImage} 
          alt={title}
        ...
        {/* Sử dụng biến imageError (thêm vào phần hiển thị lỗi) */}
        {imageError && (
