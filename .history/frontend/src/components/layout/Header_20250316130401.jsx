  ...
  useEffect(() => {
    if (isAuthenticated && user) {
      try {
        ...
        // Thử nhiều dịch vụ avatar khác nhau
        const dicebearUrl = `https://api.dicebear.com/7.x/anime/svg?seed=${seed}`;
        
        // Dự phòng URL avatar
        const fallbackUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(seed)}&background=random`;
        
        // Kiểm tra xem DiceBear có hoạt động không
        fetch(dicebearUrl, { method: 'HEAD' })
          .then(() => {
            console.log("DiceBear accessible, using it");
            setAvatar(dicebearUrl);
          })
          .catch(() => {
            console.log("DiceBear not accessible, using fallback");
            setAvatar(fallbackUrl);
          });
      } catch (error) {
        console.error("Error setting avatar:", error);
        setAvatar(`https://ui-avatars.com/api/?name=${encodeURIComponent('User')}&background=random`);
      }
    }
