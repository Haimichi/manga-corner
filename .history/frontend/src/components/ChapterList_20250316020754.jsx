...
const ChapterList = ({ chapters = [], mangaId }) => {
  ...
  useEffect(() => {
    console.log('Chapters received in component:', chapters);
    if (chapters?.data) {
      console.log('Chapter count:', chapters.data.length);
    }
  }, [chapters]);

  if (!chapters?.data || chapters.data.length === 0) {
    console.log('No chapters found or chapters data invalid:', chapters);
    return (
      <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 p-4 rounded-md">
        <p>Không có chapter nào cho truyện này</p>
        <p className="text-sm mt-2">Chi tiết: {JSON.stringify({
          hasChapters: Boolean(chapters),
          dataType: chapters ? typeof chapters.data : 'undefined',
          isArray: chapters?.data ? Array.isArray(chapters.data) : false,
          length: chapters?.data?.length || 0
        })}</p>
      </div>
    );
  }
