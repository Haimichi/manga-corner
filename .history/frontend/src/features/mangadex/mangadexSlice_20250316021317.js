...
// Thêm action riêng để reset chapters khi cần
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchMangaChapters.pending, (state) => {
        ...
        // Reset chapters để tránh hiển thị dữ liệu cũ khi loading
        state.chapters = { data: [] };
      })
      .addCase(fetchMangaChapters.fulfilled, (state, action) => {
        ...
        if (action.payload && action.payload.data) {
          ...
          // Log ra để debug
          console.log('Cập nhật chapters:', action.payload.data.length, 'chapters');
        } else {
          // Trường hợp không có dữ liệu hợp lệ
          console.warn('Không có dữ liệu chapters hợp lệ');
          state.chapters = { data: [] };
        }
      })
      .addCase(fetchMangaChapters.rejected, (state, action) => {
        ...
        // Log lỗi để debug
        console.error('Lỗi khi lấy chapters:', action.payload);
      })
