...
export const fetchMangaChapters = createAsyncThunk(
  ...
  async (mangaId, { rejectWithValue }) => {
    try {
      ...
      const response = await mangadexApi.getChapters(mangaId);
      
      // QUAN TRỌNG: In ra toàn bộ dữ liệu trả về để debug
      console.log('Redux: Response từ API:', response);
      ...
      console.log('Redux: Chapters đã nhận:', response.data.data.length);
      
      // Trả về đúng định dạng dữ liệu (response.data)
