...
export const fetchMangaChapters = createAsyncThunk(
  ...
  async (mangaId, { rejectWithValue }) => {
    try {
      console.log('Đang lấy chapter cho manga:', mangaId);
      ...
      if (!response.data || !response.data.data) {
        console.warn('Dữ liệu chapters không hợp lệ:', response);
        return { data: [] };
      }
      
      console.log('Đã nhận được', response.data.data.length, 'chapter');
      return response.data;
    } catch (error) {
      console.error('Lỗi khi lấy chapter:', error);
      return rejectWithValue(error.message || 'Không thể tải chapter');
    }
  }
);
...
// Thêm action riêng để reset chapters
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchMangaChapters.fulfilled, (state, action) => {
        ...
        // Đảm bảo state.chapters luôn có giá trị hợp lệ
        state.chapters = action.payload && action.payload.data ? action.payload : { data: [] };
        console.log('Đã cập nhật state chapters, số lượng:', state.chapters.data?.length || 0);
      })
