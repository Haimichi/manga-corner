  ...
  extraReducers: (builder) => {
    builder
        ...
        try {
          if (action.payload && action.payload.data) {
            if (Array.isArray(action.payload.data)) {
              ...
              console.error('Cấu trúc dữ liệu không hợp lệ', action.payload);
              state.latest = [];
              state.error = 'Cấu trúc dữ liệu không hợp lệ';
            }
          } else {
            console.error('Không nhận được dữ liệu hợp lệ', action.payload);
            state.latest = [];
      ...
      .addCase(fetchLatestManga.rejected, (state, action) => {
        state.loading = false;
        state.error = action.error.message || 'Không thể tải manga mới nhất';
        state.latest = [];
      })
