  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchLatestManga.fulfilled, (state, action) => {
        state.loading = false;
        // Kiểm tra cấu trúc data trước khi cập nhật state
        if (action.payload && action.payload.data) {
          state.latest = action.payload.data;
        } else {
          console.error('Invalid data structure in fetchLatestManga response');
          state.latest = [];
      ...
      .addCase(fetchLatestManga.rejected, (state, action) => {
        ...
        console.error('Lỗi trong fetchLatestManga:', state.error);
      })
