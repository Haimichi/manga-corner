  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchLatestManga.fulfilled, (state, action) => {
        ...
        console.log('Action payload trong fetchLatestManga.fulfilled:', 
          JSON.stringify(action.payload).slice(0, 100) + '...');
        
        // Kiểm tra dữ liệu trả về
        if (!action.payload) {
          console.error('Không có dữ liệu trả về từ fetchLatestManga');
          state.latest = [];
        ...
        if (action.payload.data && Array.isArray(action.payload.data.data)) {
        ...
        } else if (action.payload.data && action.payload.data.results && Array.isArray(action.payload.data.results)) {
          // Trường hợp MangaDex API trả về dữ liệu trong "results"
          console.log('Nhận được response với cấu trúc action.payload.data.results');
          state.latest = action.payload.data.results;
          ...
          // Trường hợp action.payload.results là mảng 
          console.log('Nhận được response với cấu trúc action.payload.results mảng');
          state.latest = action.payload.results;
        } else {
        ...
        // Log về dữ liệu đã được lấy
        console.log(`Đã lấy được ${state.latest.length} manga mới nhất`);
      })
