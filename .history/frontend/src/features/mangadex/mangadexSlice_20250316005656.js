...
export const getMangaDetailAsync = createAsyncThunk(
  ...
  async (mangaId, { rejectWithValue }) => {
    try {
      ...
      console.log(`Đang gọi API lấy thông tin manga ${mangaId}...`);
      const response = await getMangaDetails(mangaId);
      console.log(`Kết quả API manga:`, response);
      
      // Kiểm tra dữ liệu trả về
      if (!response.data || !response.data.data) {
        console.error('Dữ liệu manga không hợp lệ:', response);
      ...
      return rejectWithValue(error?.response?.data?.message || 'Không thể lấy thông tin manga');
    }
  }
);
