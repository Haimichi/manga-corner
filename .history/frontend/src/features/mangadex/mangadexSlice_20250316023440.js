...
export const fetchMangaChapters = createAsyncThunk(
  ...
  async (mangaId, { rejectWithValue }) => {
    try {
      ...
      // Thêm timestamp để buộc lấy dữ liệu mới
      ...
      console.log('Redux: Mẫu dữ liệu chapter:', response.data.data[0]);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchMangaChapters.fulfilled, (state, action) => {
        ...
        console.log('Redux: State chapters đã cập nhật với', state.chapters.data.length, 'chapters');
      })
      ...
      // Xử lý resetChapters
      .addCase(resetChapters.fulfilled, (state, action) => {
        state.chapters = { data: [] };
        state.chaptersLoading = false;
        state.chaptersError = null;
      });
