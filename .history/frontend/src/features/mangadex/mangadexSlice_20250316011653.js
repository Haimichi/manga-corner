...
export const fetchMangaChapters = createAsyncThunk(
  ...
  async (mangaId, { rejectWithValue }) => {
    try {
      ...
      // Gọi API với các tham số rõ ràng
      ...
      console.log('Response từ API chapters:', response);
      
      // Kiểm tra dữ liệu và đảm bảo trả về cấu trúc đúng
    ...
    builder
      ...
      .addCase(fetchMangaChapters.fulfilled, (state, action) => {
        ...
        if (action.payload?.data?.data && Array.isArray(action.payload.data.data)) {
          ...
          console.log(`Đã load ${state.chapters.length} chapters thành công`);
        } else {
          console.warn('Dữ liệu chapter không đúng định dạng:', action.payload);
          state.chapters = [];
        }
      })
