...
export const updateManga = createAsyncThunk(
  'manga/updateManga',
  async ({ id, mangaData }, { rejectWithValue }) => {
    try {
      const response = await mangaApi.updateManga(id, mangaData);
      return response.data;
    } catch (error) {
      return rejectWithValue(error.response?.data || { message: 'Lỗi khi cập nhật manga' });
    }
  }
);

export const rejectManga = createAsyncThunk(
  'manga/rejectManga',
  async ({ id, reason }, { rejectWithValue }) => {
    try {
      const response = await mangaApi.rejectManga(id, { reason });
      return response.data;
    } catch (error) {
      return rejectWithValue(error.response?.data || { message: 'Lỗi khi từ chối manga' });
    }
  }
);

export const fetchFavorites = createAsyncThunk(
  'manga/fetchFavorites',
  async (_, { rejectWithValue }) => {
    try {
      const response = await mangaApi.getFavorites();
      return response.data;
    } catch (error) {
      return rejectWithValue(error.response?.data || { message: 'Lỗi khi tải truyện yêu thích' });
    }
  }
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateManga.fulfilled, (state, action) => {
        state.loading = false;
        state.mangaDetail = action.payload?.data;
        
        if (state.myManga && state.myManga.length > 0) {
          const index = state.myManga.findIndex(manga => manga._id === action.payload?.data._id);
          if (index !== -1) {
            state.myManga[index] = action.payload?.data;
          }
        }
      })
      .addCase(updateManga.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload?.message || action.error?.message || 'Lỗi khi cập nhật manga';
      })
      .addCase(rejectManga.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(rejectManga.fulfilled, (state, action) => {
        state.loading = false;
        if (state.pendingManga) {
          state.pendingManga = state.pendingManga.filter(manga => manga._id !== action.payload?.data._id);
        }
      })
      .addCase(rejectManga.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload?.message || action.error?.message || 'Lỗi khi từ chối manga';
      })
      ...
      .addCase(fetchFavorites.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload?.message || action.error?.message || 'Lỗi khi tải truyện yêu thích';
      });
  }
});
