...
exports.getMangaChapters = async (req, res) => {
  try {
    ...
    // Hàm để lấy tất cả chapter qua nhiều trang
    const getAllChapters = async () => {
      let allChapters = [];
      let currentOffset = parseInt(offset);
      let hasMore = true;
      const LIMIT = 100; // Max limit cho mỗi request
      
      console.log("Bắt đầu lấy tất cả chapter...");
      ...
      while (hasMore) {
        ...
        console.log(`Đang lấy chapter offset ${currentOffset} đến ${currentOffset + LIMIT}...`);
        ...
        if (response.data && response.data.data && response.data.data.length > 0) {
          ...
          // Nếu số chapter trả về ít hơn LIMIT, nghĩa là đã lấy hết
    ...
    // Lấy tất cả chapter qua nhiều trang
    const chaptersData = await getAllChapters();
    
    console.log(`Đã lấy tổng cộng ${chaptersData.data.length} chapter`);
    ...
    res.status(200).json({
      ...
      data: {
        ...
        limit: chaptersData.limit,
        offset: chaptersData.offset,
